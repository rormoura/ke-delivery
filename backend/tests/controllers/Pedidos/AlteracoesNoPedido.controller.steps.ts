...
const feature = loadFeature('tests/features/pedidos/AlteracoesNoPedido.feature', {tagFilter: "@runThis" });
...
defineFeature(feature, (test) => {
  ...
  test('Cancelar pedido (SERVICE)', ({ given, when, then, and }) => {
    ...
    when(/^uma requisição de alteração for enviada para "(.*)" com o corpo JSON contendo id = "(.*)", IdCliente = "(.*)", IdEntregador = "(.*)", IdRestaurante = "(.*)", Data = "(.*)", Endereco = "(.*)", Itens = "(.*)", MetodoDePagamento = "(.*)", Observacoes = "(.*)", Status = "(.*)" e ValorTotal = "(.*)"/,
        async (URL, id, IdCliente, IdEntregador, IdRestaurante, Data, Endereco, Itens, MetodoDePagamento, Observacoes, Status, ValorTotal) => {
  ...
  test('Adiconar comentário em um pedido inexistente(SERVICE)', ({ given, when, then, and }) => {
    given(/^PedidosService contém um pedido com id = "(.*)", IdCliente = "(.*)", IdEntregador = "(.*)", IdRestaurante = "(.*)", Data = "(.*)", Endereco = "(.*)", Itens = "(.*)", MetodoDePagamento = "Dinheiro", Observacoes = "(.*)", Status = "(.*)" e ValorTotal = "(.*)"/,
      async (id, IdCliente, IdEntregador, IdRestaurante, Data, Endereco, Itens, MetodoDePagamento, Observacoes, Status, ValorTotal) => {
        mockPedidoEntity = await mockPedidoRepository.createPedido(new PedidoEntity({
          "id": id,
          "IdCliente": IdCliente,
    ...
    when(/^uma requisição PUT for enviada para "(.*)" com o corpo JSON contendo id = "(.*)", IdCliente = "(.*)", IdEntregador = "(.*)", IdRestaurante = "(.*)", Data = "(.*)", Endereco = "(.*)", Itens = "(.*)", MetodoDePagamento = "Dinheiro", Observacoes = "(.*)", Status = "(.*)" e ValorTotal = "(.*)"/,
      async (URL, id, IdCliente, IdEntregador, IdRestaurante, Data, Endereco, Itens, MetodoDePagamento, Observacoes, Status, ValorTotal) => {
        response = await request.put(URL).send({
          "id": id,
          "IdCliente": IdCliente,
  ...
  test('Adiconar comentário em um pedido existente (SERVICE)', ({ given, when, then, and }) => {
    ...
    when(/^uma requisição PUT for enviada para "(.*)" com o corpo da requisição JSON contendo id = "(.*)", IdCliente = "(.*)", IdEntregador = "(.*)", IdRestaurante = "(.*)", Data = "(.*)", Endereco = "(.*)", Itens = "(.*)", MetodoDePagamento = "(.*)", Observacoes = "(.*)", Status = "(.*)" e ValorTotal = "(.*)"/,
      async (URL, id, IdCliente, IdEntregador, IdRestaurante, Data, Endereco, Itens, MetodoDePagamento, Observacoes, Status, ValorTotal) => {
        response = await request.put(URL).send({
          "id": id,
          "IdCliente": IdCliente,
